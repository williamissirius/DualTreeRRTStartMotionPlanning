    ...
    for speedi in linspace(3, 18, num = 10): # sample speed at the narrow point
        ...
        for i in range(0,1): # sample trajectory at the state with specific speed 
            ...
            while t1<tforward:
                ...
                tempV = (statet[3]**2.0 + statet[4]**2.0 + statet[5]**2.0)**(1.0/2.0) ;
                ...
                fTotal4 = [f[3]] + fTotal4;
                transV = [tempV] + transV;
                zAxisAnlge = np.arccos(us.Q2R(statet[6:10])[2,2]);
                zError = [zAxisAnlge/3.1416*180] + zError;
                # print "zAngle " + str(zAngle) + " speedTotal " + str(speedTotal)
                if abs((zAngle ))<0.1 and speedTotal < 1:
                    fFind = 1
                    break
            ...
            if fFind == 1:
                ...
                while t2<tforward:
                    ...
                    transV.append(tempV);
                    zAxisAnlge = np.arccos(us.Q2R(statet[6:10])[2,2]);
                    zError.append(zAxisAnlge/3.1416*180);
                    # if env.CheckCollision(robot) == True:
                    #     break 

                    # print "zAngle " + str(zAngle) + " speedTotal " + str(speedTotal)
                    if abs((zAngle ))<0.1 and speedTotal < 2:
                        bFind = 1
                        break
                        print "find stable transition point"
                    t2 = t2 - delta_t
        ...
        # plt.plot(fTotal4)
        # plt.show()
    return [ctotal,fTotal1,fTotal2,fTotal3,fTotal4,transV,zError,len(cb),len(cf)]
...
if __name__ == "__main__":
    ...
    with env:
        ...
        print NarrPtNeg
        print NarrPtPos
    # raw_input("record the narrow points")
    ...
    ##################################
    ## plot the local path analysis ##
    ################################## 

    axistime = np.arange(0,len(temp1[1])*0.01,0.01)

    # fig, ax = plt.subplots()
    # print axistime
    # print len(list(axistime))
    # print len(temp1[1])
    # ax.plot(list(axistime),temp1[1], label='propeller 1')
    # ax.plot(list(axistime),temp1[2], label='propeller 2')
    # ax.plot(list(axistime),temp1[3], label='propeller 3')
    # ax.plot(list(axistime),temp1[4], label='propeller 4')
    ...
    ax1.plot(list(axistime),temp1[1], label='propeller 1')
    ax1.plot(list(axistime),temp1[2], label='propeller 2')
    ax1.plot(list(axistime),temp1[3], label='propeller 3')
    ax1.plot(list(axistime),temp1[4], label='propeller 4')
    legend1 = ax1.legend(loc='upper left', shadow=False, ncol=2)
    ...
    for label in legend1.get_lines():
        label.set_linewidth(1.5)  # the legend line width
    ax1.set_ylabel('$F$ (N)', fontsize=15)
    ax1.tick_params(labelsize=15)


    # share x only
    ax2 = plt.subplot(312, sharex=ax1)
    ax2.plot(list(axistime),temp1[5], label='translational velocity')
    plt.setp(ax2.get_xticklabels(), visible=False)
    ax2.tick_params(labelsize=15)
    ax2.set_ylabel("$V_t$ (m/s)", fontsize=15)
    # share x and y
    ax3 = plt.subplot(313, sharex=ax1)
    ax3.plot(list(axistime),temp1[6], label='z axis')
    plt.setp(ax2.get_xticklabels(), visible=False)
    ax3.set_ylabel("$Z$ ($\degree$)", fontsize=15)
    ax3.tick_params(labelsize=15)

    narrowTime = 0.01*temp1[8];
    print narrowTime
    # ax3.annotate('time through narrow passage',
    #         xy=(narrowTime, 10), xycoords='data',
    #         xytext=(narrowTime, 30), textcoords='data',
    #         arrowprops=dict(facecolor='black', shrink=0.05),
    #         horizontalalignment='right', verticalalignment='bottom')

    ax1.axvline(narrowTime, color='red', linestyle='--',linewidth=2)
    ax2.axvline(narrowTime, color='red', linestyle='--',linewidth=2)
    ax3.axvline(narrowTime, color='red', linestyle='--',linewidth=2)


    ax3.annotate('$t_{nar}$', 
            xy=(narrowTime+0.4, 10), 
            xytext=(narrowTime+0.4, 10),
            fontsize=15)
    ...
    '''
    PathFinal = NarrPathNeg;
    PathFinal = NarrPathPos;

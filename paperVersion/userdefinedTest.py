...
def sampleFSstillBack(s,stdev = 0.6,attitude = 0):
	...
	# average = average + eXz*kz
	# force for position control k
	f1 = np.random.normal(average + 0.5*kx*eXx, stdev)
	f2 = np.random.normal(average + 0.5*ky*eXy, stdev)
	f3 = np.random.normal(average - 0.5*kx*eXx, stdev)
	f4 = np.random.normal(average - 0.5*ky*eXy, stdev)

	# f1 = np.random.normal(average , stdev)
	# f2 = np.random.normal(average , stdev)
	# f3 = np.random.normal(average , stdev)
	# f4 = np.random.normal(average , stdev)


	# force to control the attitude
	# the position of rotor in inertia frame

	# force for aititude control
	# p control
	rp1 = np.matmul(R,np.array([0.2,0,0]))[2]**1*krp
	rp2 = np.matmul(R,np.array([0,0.2,0]))[2]**1*krp
	rp3 = np.matmul(R,np.array([-0.2,0,0]))[2]**1*krp
	rp4 = np.matmul(R,np.array([0,-0.2,0]))[2]**1*krp
	...
	print 'altitude force : ',r1,r2,r3,r4,'  position pforce : ', f1,f2,f3,f4 



	f = [min(10,max(0,f1+r1)),min(10,max(0,f2+r2)),min(10,max(0,f3+r3)),min(10,max(0,f4+r4))]
	if attitude == 1:
		f =[min(10,max(0,rp1)),min(10,max(0,rp2)),min(10,max(0,r3)),min(10,max(0,r4))]
...
def sampleFSC(s,c,stdev = 0.6,attitude = 0):
	# sample force vector at state s while setting c as the goal
	# error is the position error with the goal
	...
	eXb = np.matmul(R.T,dX - X) #error on X in body Frame
	eVb = np.matmul(R.T,dV - V)
	eXb = eVb*kv + eXb
	...
	print 'altitude force : ',r1,r2,r3,r4
	print 'position pforce : ', f1,f2,f3,f4 



	f = [f1+r1,f2+r2,f3+r3,f4+r4]
	if attitude == 1:
		f =[rp1,rp2,r3,r4]
...
def sampleFSstill(s,stdev = 0.6,attitude = 0):
	# sample force vector at state s while setting c as the goal
	# error is the  position error with the goal
...
def Q2E(Q):
	return tf.euler_from_quaternion(Q, axes='rzxz')

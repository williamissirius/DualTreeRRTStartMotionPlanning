...
def pathNearNarrowPoint(NarrPt):
    ...
    for speedi in linspace(1, 18, num = 100): # sample speed at the narrow point
        statei = NarrPt[0:3]+[-speedi,0,0] + NarrPt[3:7]+[0,0,0]
        for i in range(0,100): # sample trajectory at the state with specific speed 
            iterTotal = 3
            delta_t = 0.01
            statet = statei    
            tbackward = -3
            ...
            while t1<tforward:
                ...
                xAngle = us.Q2E(statet[6:10])[0];
                yAngle = us.Q2E(statet[6:10])[2];
                totalAngle = abs(zAngle) + abs(xAngle) + abs(yAngle)
                speedTotal = abs(statet[3]) + abs(statet[4]) + abs(statet[5]);
...
if __name__ == "__main__":
    ...
    with env:
        ...
        #########################################
        #########################################
        # NarrPtNeg = [-1.89,0.8,1.1,0.971,0.17,-0.1,0.13];
        # NarrPtPos = [2.1,-0.78,1.09,0.97,-0.17,0.12,0.15];
        NarrPtNeg = [-2,0.8,1.1,-0.8,0,0.2,-0.5];
        NarrPtPos = [2,-0.8,1.2,-0.5,0.2,0,0.8];

    temp1 = pathNearNarrowPoint(NarrPtNeg)
    NarrPathNeg = temp1[0]
    temp2 = pathNearNarrowPoint(NarrPtPos)
    NarrPathPos = temp2[0]


    speedTrans = [];
    speedTrans = speedTrans + [np.linalg.norm(temp1[1][0][3:6])];
    speedTrans = speedTrans + [np.linalg.norm(temp1[1][-1][3:6])];
    speedTrans = speedTrans + [np.linalg.norm(temp2[1][0][3:6])];
    speedTrans = speedTrans + [np.linalg.norm(temp2[1][-1][3:6])];
    ...
    for i in range(len(Npath)):
        ...
        pathSegment = pathSegment + [us.stepNodesQ(beginNode,NpathiBegin,stepFinal),Npahti];
        beginNode = NpahtiEnd
    PathFinal = PathFinal + us.stepNodesQ(beginNode,goalconfig,stepFinal)
    pathSegment = pathSegment + [us.stepNodesQ(beginNode,goalconfig,stepFinal)]
    print PathFinal
    print type(PathFinal)
    handles = []
    plotPath(PathFinal,handles)

    print [len(pathSegment[0]),len(pathSegment[1]),len(pathSegment[2]),len(pathSegment[3]),len(pathSegment[4])]
    ...
    # dx = [0.5]
    # for i in xrange(len(PathFinal)-1):
    #     dx = dx + [np.linalg.norm(  np.array(PathFinal[i][0:3]) -np.array(PathFinal[i+1][0:3]) )]
    #     print dx[i]

    
    # for i, wayPoint in enumerate(PathFinal):

    #     traj.Insert(i, wayPoint+[0.01*i],config,True);

    #####################################################
    ############## draw path according to real speed #### speedTrans
    #####################################################

    speedBegin = [speedTrans[i] for i in [0,1,3]];
    speedEnd = [speedTrans[i] for i in [0,2,3]];

    pathStraight = [pathSegment[i] for i in [0,2,4]];
    ...
    timeStraight = [[],[],[]];
    for i in xrange(3):
        dis = len(pathStraight[i])*dx;
        p = 0;
        q = len(pathStraight[i]) - 1;
        r = p*0.5 + q*0.5;
        b = speedBegin[i];
        e = speedEnd[i];
        for j in xrange(len(pathStraight[i])):
            v = (j-q)*(j-r)/((p-q)*(p-r))*b + (j-p)*(j-q)/((r-q)*(r-p))*r + (j-p)*(j-r)/((q-p)*(q-r))*e; 
            timeStraight[i] = timeStraight[i] + [dx/v];

    tControl = 0.0001;
    timeFinal = timeStraight[0] + len(pathSegment[1])*[tControl] +timeStraight[1] + len(pathSegment[3])*[tControl] +timeStraight[2];

    timeFinal = np.cumsum(timeFinal);
    ...
    for i, wayPoint in enumerate(PathFinal):
        traj.Insert(i, wayPoint+[timeFinal[i]],config,True);
